...
public class CommonsTests {
    ...
    void test_gameInProgress_False_Not_Started() throws Exception {
        assertEquals(false, Commons.builder().startingDate(end).lastDate(end2).build().gameInProgress());
    }
    @Test
    void test_gameInProgress_False_Already_Ended() throws Exception {
        assertEquals(false, Commons.builder().startingDate(start).lastDate(start2).build().gameInProgress());
    }
}